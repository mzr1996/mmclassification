...
class MultiOptimWrapperConstructor:
    ...
    def __init__(self, optim_wrapper_cfg: dict, paramwise_cfg=None):
        ...
        for key, cfg in self.optim_cfg.items():
            ...
            constructor_type = cfg_.pop('constructor',
                                        'DefaultOptimWrapperConstructor')
            self.constructors[key] = OPTIM_WRAPPER_CONSTRUCTORS.build(
                dict(
                    type=constructor_type,
                    optim_wrapper_cfg=cfg_,
                    paramwise_cfg=paramwise_cfg_))
